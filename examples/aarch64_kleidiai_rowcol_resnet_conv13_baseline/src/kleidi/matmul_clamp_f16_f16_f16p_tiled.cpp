...
#define TILED 0
#define ABFT 0
#define EPSILON 81.75f
#define ROW_EPSILON 3.0f
#define COL_EPSILON 0.12f
#define PROFILING 0
...
#include "conv_filters_resnet.hpp"

// Results structure to store execution results
struct ExecutionResults
{
    uint8_t abft_triggered_hari;       // Flag indicating if ABFT from Hari was triggered
    uint8_t abft_triggered_huang; // Flag indicating if ABFT from Huang was triggered
...
ExecutionResults execution_results = {0, 0, 0, 0, 0, 0};
...
float16_t bias[3136] = {0};
float16_t ic[3136] = {0};
float16_t rhs_packed[75000] = {0};
float16_t fc_packed[75000] = {0};
float16_t output_checksum_row[3136] = {0.0f};
float16_t output_checksum_col[3136] = {0.0f};
float16_t checksum_row[3136] = {0.0f};
float16_t checksum_col[3136] = {0.0f};
...
int run_test_case(const TestCase &test_case)
{
    ...
    const size_t rhs_packed_size_row = kai_get_rhs_packed_size_rhs_pack_kxn_f16p16x1biasf16_f16_f16_neon(1, K);

    const size_t nr_ = ukernel.get_nr();
    const size_t kr_ = ukernel.get_kr();
    const size_t sr_ = ukernel.get_sr();

    kai_run_rhs_pack_kxn_f16p16x1biasf16_f16_f16_neon(
        1, 1, K, nr_, kr_, sr_, // Packing arguments
        sizeof(float16_t),   // RHS stride
        fc,                  // RHS
        bias,                // Bias
        NULL,                // Scale
        fc_packed,           // RHS packed
        0, NULL);
    ...
    const uint8_t *lhs_ptr_ = (const uint8_t *)ic;
    const uint8_t *rhs_ptr_ = (const uint8_t *)rhs_packed + (ukernel.get_rhs_packed_offset(0, K));
    uint8_t *dst_ptr_ = (uint8_t *)(&(checksum_row[0]));
    ...
    const float16_t *lhs_actual = &(lhs[0 * K]);
    const uint8_t *lhs_ptr_2 = (const uint8_t *)(lhs_actual);
    const uint8_t *rhs_ptr_2 = (const uint8_t *)fc_packed;
    dst_ptr_ = (uint8_t *)(&(checksum_col[0]));

    ukernel.run_matmul(
        M, 1, K,          // Dimensions
        lhs_ptr_2,        // LHS
        lhs_stride,       // LHS stride
        rhs_ptr_2,        // RHS packed
        dst_ptr_,         // DST
        sizeof(uint16_t), // DST stride (row)
        sizeof(uint16_t), // DST stride (col)
        -FLT_MAX, FLT_MAX // Min and max for the clamp operation
    );
    ...
    for (size_t row = 0; row < M; row++)
    {
        if (__builtin_fabsf(output_checksum_col[row] - checksum_col[row]) > COL_EPSILON)
        {
            col_error_detected = true;
            // printf("Column Checksum Mismatch at %zu (Delta: %f)\n", row, output_checksum_col[row] - checksum_col[row]);
        }
        execution_results.max_diff = std::max(execution_results.max_diff, __builtin_fabsf(output_checksum_col[row] - checksum_col[row]));
    }

    for (size_t col = 0; col < N; col++)
    {
        if (__builtin_fabsf(output_checksum_row[col] - checksum_row[col]) > ROW_EPSILON)
        {
            row_error_detected = true;
            // printf("Row Checksum Mismatch at %zu (Delta: %f)\n", col, output_checksum_row[col] - checksum_row[col]);
        }
        execution_results.max_diff = std::max(execution_results.max_diff, __builtin_fabsf(output_checksum_row[col] - checksum_row[col]));
    }
    ...
    // Check if error was detected from Row/Col Checksums (Huang)
    if (row_error_detected || col_error_detected)
    {
        overall_error_detected = true;
        ret = 1;
        execution_results.abft_triggered_huang = 1;
    }

    // Check if error was detected from FIC Checksum (Hari)
    if (__builtin_isnan(oc) || __builtin_isnan(dot) || __builtin_fabsf(dot - oc) > EPSILON)
    {
        overall_error_detected = true;
        ret = 1;
        execution_results.abft_triggered_hari = 1;
        execution_results.abft_triggered_huang = 1;
    }

    // Update execution results
    if (overall_error_detected)
    {
...
int main(int argc, char **argv)
{
    ...
    execution_results.abft_triggered_hari = 0;
    execution_results.abft_triggered_huang = 0;
